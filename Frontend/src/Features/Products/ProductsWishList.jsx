...
export default function ProductsWishList() {
  return (
    <>
        ...
        <Header />
        <Navbar />
        <PageHeader title='wishlist' bCrumbItem1 ='Home' bCrumbActive = 'wishlist'/>
        <WishList />
        <Footer />
        <CopyRight />
    </>
...
function WishList() {
    const { wishList, removeToWishList } = UseProducts();
    ...
    return !wishList.length ? (
        <Empty text="There are no products in your wishlist!"/>
    ) : (
                ...
                <div className='row'>
                    ...
                    {wishList.map((item) => (
                        <div key={item.id} className='d-flex justify-content-between align-items-center p-3 pe-4' style={{borderBottom: '1px solid rgb(235, 235, 235) '}}>
                            ...
                            <button className="btn-remove me-5" onClick={() => removeToWishList(item)}>
